...
?>
    ...
    <head>
        ...
        <title><?php echo $_LANG['solusvmplus_html5Console']; ?></title>
        <style>
...
if (!$ca->isLoggedIn()) {
    if ((!isset($_SESSION['adminid']) || ((int)$_SESSION['adminid'] <= 0))) {
        echo '<div class="alert alert-danger">' . $_LANG['solusvmplus_unauthorized'] . '</div></body>';
        exit();
    }
    $uid = (int)$_GET['uid'];
} else {
...
if ($params === false) {
    echo '<div class="alert alert-danger">' . $_LANG['solusvmplus_vserverNotFound'] . '</div></body>';
    exit;
}
$solusvm = new SolusVM($params);

if ($solusvm->getExtData("clientfunctions") == "disable") {
    echo '<div class="alert alert-danger">' . $_LANG['solusvmplus_functionDisabled'] . '</body>';
    exit;
}
...
if ($r["status"] == "success") {
    if ($r["sessionactive"] == "1") {
        ...
        <div align="center">
            <p style="padding-bottom: 5px"><?php echo $_LANG['solusvmplus_password'] . ': ' . $r['consolepassword']; ?></p>
            <br>
        </div>
    <br>
        <form action="" method="post" name="cancelsession">
            <div style="padding: 10px 0 10px 0; text-align:center">
                <input name="sessioncancel" type="submit" value="<?php echo $_LANG['solusvmplus_cancelSession']; ?>">
            </div>
        </form>
        ...
        <div
            ...
            <div
                ...
                <div style="float: right;margin-left: 10px; cursor: pointer">
                    <i title="<?php echo $_LANG['solusvmplus_quit']; ?>" onclick="window.close();"
                       class="fa fa-times-circle"></i>
                </div>
                <div style="float: right;margin-left: 10px ; cursor: pointer">
                    <i title="<?php echo $_LANG['solusvmplus_reconnect']; ?>"
                       onclick=" open_terminal({connectionkey: '<?= $r['key']; ?>'});"
                       class="fa fa-refresh"></i>
                </div>
                <div style="float: right;margin-left: 10px; cursor: pointer">
            <span style="color: #02FC1E"><i title="<?php echo $_LANG['solusvmplus_greenText']; ?>"
            ...
            <div class="col-xs-8">
                <form class="form-inline" method="post" name="createsession">
                    <div class="form-group">
                        <label for="sessiontime"><?php echo $_LANG['solusvmplus_time']; ?></label>
                        <select class="form-control" name="sessiontime" id="sessiontime">
                            <option value="1">1 <?php echo $_LANG['solusvmplus_hour']; ?></option>
                            <option value="2">2 <?php echo $_LANG['solusvmplus_hours']; ?></option>
                            <option value="3">3 <?php echo $_LANG['solusvmplus_hours']; ?></option>
                            <option value="4">4 <?php echo $_LANG['solusvmplus_hours']; ?></option>
                            <option value="5">5 <?php echo $_LANG['solusvmplus_hours']; ?></option>
                            <option value="6">6 <?php echo $_LANG['solusvmplus_hours']; ?></option>
                            <option value="7">7 <?php echo $_LANG['solusvmplus_hours']; ?></option>
                            <option value="8">8 <?php echo $_LANG['solusvmplus_hours']; ?></option>
                        </select>
                    </div>
                    <button name="sessioncreate" type="submit"
                            class="btn btn-success"><?php echo $_LANG['solusvmplus_createSession']; ?></button>
                    <button type="button" class="btn btn-danger"
                            onclick="window.close()"><?php echo $_LANG['solusvmplus_closeWindow']; ?></button>
                </form>
            </div>
            <div class="col-xs-2"></div>
        </div>
    ...
    if (isset($r["statusmsg"])) {
        ...
        $pagedata = $_LANG['solusvmplus_couldntConnectMaster'];
    }
    $pagedata .= '<br><br>';
